...
# Suggested citation: Hu X,  Yu Y,Daly-Grafstein D, Khan M, Erdelyi S, Staples JA. AMA-OD_cco sensitivity analyses for BMA discharges - Part II. 2023 Nov 16. Retrieved from: ***LINK*** 
# Sensitivity analysis (2) (DC-BMA)
# alternate exposure
# alternate cohort
...
############# cohort = od+hosp in pre-od and pre-control, control = t0 - 1y, and only adjusted for dc_bma ############
...
unadj = clogit(case ~ dc_bma + strata(episode_id), 
               hosp_cohort_1y,
               cluster = moh_study_id,
               method  = "efron",
               robust = T)
adj = clogit(as.formula(gsub(paste("discharge","+"),"", paste("case ~ dc_bma", sub(".*~ ","",cco_adj$formula)[3]))), 
             data = hosp_cohort_1y,
             cluster = moh_study_id,
             method  = "efron",
             robust = T)
...
res_dcbma <- get_pasted_col(cohort,unadj,adj,"dc_bma_nohosp")
...
#combine results
od_res <- readRDS("sensitivity_analyses_res_part1.rds") %>% select(-estimate,-robust.se,-conf.low,-conf.high)
od_res <- rbind(od_res,
...
#save 
wb <- loadWorkbook("R:/working/AMA-OD_cco/NH/results/Table D/AMA-OD_cco - Table 4 v4.xlsx")
#sensitivity analyses
addWorksheet(wb, "Table 4 1220")
writeData(wb,"Table 4 1220", res %>% select(-fontface,-Label),rowNames = F)
...
addStyle(wb,"Table 4 1220", fontface, rows = cells_to_style$rows,cols = cells_to_style$cols)

#gender interaction term
addWorksheet(wb,"gender interaction 1219")
writeData(wb,"gender interaction 1219",get_coef(sex_inter))

#save
saveWorkbook(wb, "R:/working/AMA-OD_cco/NH/results/Table D/AMA-OD_cco - Table 4 v4.xlsx",overwrite = T) 
