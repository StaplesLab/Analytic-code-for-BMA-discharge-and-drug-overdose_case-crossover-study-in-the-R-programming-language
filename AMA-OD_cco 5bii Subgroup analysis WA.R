...
# Suggested citation: Hu X, Yu Y, Khan M, Daly-Grafstein D, Erdelyi S, Staples JA. AMA-OD_cco subgroup analysis for discharges with approval. 2023 Sep 29. Retrieved from: ***LINK*** 
# Subgroup analysis (dc-wa)
# Author: Nicole Hu 
# Date: 2023-09-25
# Updated: 2023-09-29
##########################################################
get_pasted_col <- function(df,unadj_model, adj_model, label){
  res <- 
    cbind(
      ...
      df %>% filter(interval == "od",dc_wa == 1) %>% nrow(),
      df %>% filter(interval == "control_1",dc_wa == 1) %>% nrow(),
      df %>% filter(interval == "control_2",dc_wa == 1) %>% nrow(),
      df %>% filter(interval2 == "control",dc_wa == 1) %>% nrow(),
  ...
  colnames(res) <- c("Label","OD number","WA in pre-od","WA in control 1","WA in control 2","WA in pooled control","Unadjusted OR","Adjusted OR","estimate","robust.se","conf.low","conf.high")
  return(res)
}
...
od_res
saveRDS(od_res,"sub_od_res_wa.rds")


###################### forest plot ###########################################
plot_res <- readRDS("sub_od_res_wa.rds") %>% select(Label,estimate,robust.se,conf.low,conf.high)
...
saveRDS(plot_res,"plot_res_wa.rds")
plot_res <- readRDS("plot_res_wa.rds")
...
ggsave("R:/working/AMA-OD_cco/NH/results/fig/cco_forest_wa v1.tiff", 
       width=10, height=nrow(plot_res)*0.2 + 0.25, units="in", dpi=600, compression = "lzw", 
       plot = plot_subgroup)
ggsave("R:/working/AMA-OD_cco/NH/results/fig/cco_forest_wa v1.pdf", 
...
table_res <- read.xlsx("label_name.xlsx") %>% select(label,lab_show,fontface) %>% 
  left_join(readRDS("sub_od_res_wa.rds") %>% select(-estimate,-robust.se,-conf.low,-conf.high), by = c("label" = "Label")) %>% 
  filter(!is.na(label)) %>% 
  select(-label)


wb <- createWorkbook("R:/working/AMA-OD_cco/NH/results/Table C/AMA-OD_cco - Table 3 WA v1.xlsx")
addWorksheet(wb, "Table 3 wa")
writeData(wb,"Table 3 wa", table_res %>% select(-fontface),rowNames = F)
...
addStyle(wb,"Table 3 wa", fontface, rows = cells_to_style$rows,cols = cells_to_style$cols)
saveWorkbook(wb, "R:/working/AMA-OD_cco/NH/results/Table C/AMA-OD_cco - Table 3 WA v1.1.xlsx",overwrite = T) 
